...
Console.Write("Минимальное число в матрице: ");
int min = Convert.ToInt32(Console.ReadLine());
Console.Write("Максимальное число в матрице: ");
...
double[,] CreateMatrixRndInt(int row, int col, int min, int max)
{
    ...
    for (int i = 0; i < matrix.GetLength(0); i++)
    {
        ...
        {
           matrix[i, j] = Convert.ToDouble(rnd.Next(min*10, max*10))/10;
        }
    }
    return matrix;
}
...
double[,] array2D = CreateMatrixRndInt(row, col, min, max);
PrintArray(array2D);
